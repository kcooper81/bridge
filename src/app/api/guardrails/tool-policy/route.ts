  ...
  try {
    ...
    if (currentSettings.cloudflare_account_id && currentSettings.cloudflare_api_token) {
      ...
      if (verification.success) {
        ...
        const syncResult = await syncBlockedTools(config, blockedToolIds);
        if (syncResult.success) {
          cloudflareResult = { created: syncResult.created, deleted: syncResult.deleted, errors: syncResult.errors };
