  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        a: "Cloudflare Gateway provides network-level AI tool blocking. The full setup has 4 parts:\n\n**Part 1 — Create a Cloudflare account (if you don't have one)**\n1. Go to dash.cloudflare.com/sign-up\n2. Sign up with Google, GitHub, or Apple (free)\n3. Select the free Zero Trust plan (up to 50 users)\n\n**Part 2 — Connect TeamPrompt to Cloudflare**\n1. In Cloudflare: go to dash.cloudflare.com/profile/api-tokens\n2. Click 'Create Token' → scroll down → 'Create Custom Token'\n3. Name: 'TeamPrompt'\n4. Permission: Account → Zero Trust → Edit\n5. Account Resources: Include → All accounts\n6. Create Token → copy it\n7. In TeamPrompt: Settings → Integrations → Cloudflare Gateway → Connect\n8. Paste your Account ID (from Cloudflare dashboard home page) and the API token\n\n**Part 3 — Choose which AI tools to approve/block**\nThe wizard shows all 18 AI tools. Check/uncheck to approve or block. TeamPrompt creates DNS rules in Cloudflare automatically.\n\n**Part 4 — Deploy WARP to your team's devices**\nCloudflare Gateway rules ONLY work on devices running Cloudflare WARP. Without WARP, DNS queries bypass Gateway entirely.\n\n1. In Cloudflare Zero Trust: complete the device setup wizard (choose 'DNS only' mode — not 'Both Traffic and DNS' which requires certificate installation and can break apps. TeamPrompt's extension handles content scanning, Cloudflare only needs DNS blocking)\n2. Set up enrollment policy: Settings → WARP Client → Device enrollment → Add rule → Emails ending in '@yourcompany.com' → Allow\n3. Have each team member: download WARP from one.one.one.one, open it, go to Settings → Account → Login to Zero Trust, enter your team name, authenticate\n4. For managed devices: use MDM deployment (Intune, JAMF, Google Admin) to push WARP silently\n\nOnce WARP is enrolled, blocked AI tools show a Cloudflare block page instead of loading.",
        keywords: ["cloudflare", "gateway", "dns", "network", "integration", "warp", "zero trust", "setup", "complete guide"],
      },
      {
