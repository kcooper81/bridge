...
CREATE INDEX idx_chat_tags_user ON chat_tags(user_id);
ALTER TABLE chat_tags ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users manage own tags" ON chat_tags
  FOR ALL USING (user_id = auth.uid());
...
CREATE INDEX idx_chat_presets_org ON chat_presets(org_id);
